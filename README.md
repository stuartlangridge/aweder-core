...
#### GrumPHP
This will be running after app install, this sets up a pre-commit hook to make sure
all the code being submitted is standard compliant. This will only work when the docker container
is running.
To turn this off you can run
```
composer run grumphp-deinit
```
To turn this back on 
```
composer run grumphp-init
```
Its always worth keeping this on to make sure the code is compliant.

#### NPM
To install JS dependencies and compile CSS with webpack, run npm outside of the Docker container(s) in the project root.
